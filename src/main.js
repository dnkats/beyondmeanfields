...
const PRESETS = {
  ...
  high:   { high: true,  post: true,  ao: true,  bloom: true,  shadow: 2048, pr: 1.25, grass: 8000, trees: 1.0, lodNear: 40, lodFar: 110, impFar: 420, terrainN: 400 },
  medium: { high: false, post: true,  ao: false, bloom: true,  shadow: 2048, pr: 1.0,  grass: 5000, trees: 0.7, lodNear: 28, lodFar: 80,  impFar: 420, terrainN: 300 },
  low:    { high: false, post: false, ao: false, bloom: false, shadow: 1024, pr: 1.0,  grass: 2500, trees: 0.5, lodNear: 20, lodFar: 60,  impFar: 400, terrainN: 220 },
};
...
async function build() {
  ...
  const T = quality.trees, TA = T * 4;   // TA for island-wide scatters: the island has four times the area it was designed with; zone-local scatters keep T
  // vegetation
  const jobs = [];
  ...
  const tree = (name, n, opts, collide = 0.3) => jobs.push(scatterModel(scene, name, placements(Math.round(n * (opts.near ? T : TA)), { zoneMargin: 1.3, ...opts }), { foliage: true, impostor: true, collide }));   // crowns stay clear of the zone rims so the camera is not buried in leaves
  ...
  jobs.push(scatterModel(scene, 'q_bush_flowers', placements(Math.round(150 * TA), { scale: [0.6, 1.1], zoneMargin: 0.6, seedOffset: 11 }), { foliage: true, shadows: false }));   // the kit's plain bush is red-leaved; the flowering one is green
  ...
  jobs.push(scatterModel(scene, 'coast_rocks_02', placements(70, { minH: 0.2, maxH: 1.2, maxSlope: 2, scale: [0.5, 1.0], zoneMargin: 2.2 }), { shadows: true, field: true }));   // a set spans 10 m from its origin: keep the origin well outside the zones
  ...
  const TABLE_H = 0.55;   // the workbench top (WoodenTable_01 is 0.55 m high)
  props.push(placeModel(scene, 'chemistry_set', bz.x - 2.3, bz.z + 4, { y: null, rot: 0.3 }).then((o) => { o.position.y += TABLE_H; }), placeModel(scene, 'bunsen_burner', bz.x - 1.4, bz.z + 4.2, {}).then((o) => { o.position.y += TABLE_H; }),
    placeModel(scene, 'vintage_microscope', bz.x - 2.9, bz.z + 3.7, { rot: 1 }).then((o) => { o.position.y += TABLE_H; }), placeModel(scene, 'book_encyclopedia_set_01', bz.x - 1.2, bz.z + 3.6, { rot: 1.6 }).then((o) => { o.position.y += TABLE_H; }),
  ...
  props.push(cottage(scene, fl.x + 9, fl.z + 4, { w: 5.2, d: 4.4, rot: -0.6, roof: 'reed_roof_03', wall: 'plastered_wall_02' }), placeModel(scene, 'stone_fire_pit', fl.x + 3, fl.z + 8, {}), placeModel(scene, 'wooden_bucket_01', fl.x + 5, fl.z + 6, {}), placeModel(scene, 'wooden_ladder', fl.x + 11.2, fl.z + 1.6, { rot: 0.9 }));
  ...
  props.push(placeModel(scene, 'coastal_cliff_02', cv.x + 8, cv.z - 6, { rot: 0.6, scale: 0.9, field: true }), placeModel(scene, 'rock_face_01', cv.x + 3, cv.z - 10, { rot: 2.2, scale: 1.1, field: true }), placeModel(scene, 'treasure_chest', cv.x + 12.5, cv.z - 9, { rot: 2.4 }), placeModel(scene, 'wooden_lantern_01', cv.x - 2, cv.z + 4, {}));
  // the archive door at the cave mouth (the door model stands on y = 0, no lift); E opens the grimoire there
  const archiveDoor = await placeModel(scene, 'large_castle_door', cv.x + 6.6, cv.z - 3.9, { rot: -0.57 });   // right at the cliff wall
  ...
  props.push(tower(scene, tw.x, tw.z - 4, tw.h), placeModel(scene, 'large_castle_door', tw.x, tw.z + 0.4, { rot: 0 }), placeModel(scene, 'cannon_01', tw.x + 6, tw.z + 2, { rot: 2.4 }), placeModel(scene, 'wooden_lantern_01', tw.x - 4, tw.z + 3, {}));
  ...
  props.push(cottage(scene, lg.x - 6, lg.z + 5, { w: 5.4, d: 4.4, rot: 0.9, roof: 'reed_roof_03', wall: 'brown_planks_09', timber: false }), placeModel(scene, 'wooden_barrels_01', lg.x + 3, lg.z + 6, { rot: 1.1 }), placeModel(scene, 'wooden_crate_01', lg.x + 5, lg.z + 4, { rot: 0.4 }), placeModel(scene, 'wooden_crate_01', lg.x + 5.8, lg.z + 5.2, { rot: 1.2 }),
    placeModel(scene, 'wooden_ladder', lg.x - 8.2, lg.z + 8.2, { rot: 0.9 }), placeModel(scene, 'wooden_lantern_01', lg.x + 1, lg.z - 3, {}), placeModel(scene, 'wooden_stool_01', lg.x + 2, lg.z - 1, {}),
    placeModel(scene, 'ship_pinnace', lg.x - 12, lg.z - 16, { y: WATER_Y - 0.25, rot: 2.2, scale: 0.16, onGround: false }));
  // Transcorrelated Marsh: a fire, lanterns on poles, kit rocks at the pools
  props.push(placeModel(scene, 'stone_fire_pit', ms.x + 2, ms.z + 4, {}), placeModel(scene, 'wooden_lantern_01', ms.x - 4, ms.z - 2, {}), placeModel(scene, 'wooden_lantern_01', ms.x + 7, ms.z - 5, {}), placeModel(scene, 'wooden_crate_01', ms.x - 6, ms.z + 5, { rot: 0.4 }), placeModel(scene, 'wooden_bucket_01', ms.x + 3, ms.z + 6, {}));
  [[12, -6, 1.4], [6, -18, 1.2], [18, -16, 1.1], [-16, -12, 1.3], [-6, -22, 1.0], [-8, -11, 0.9]].forEach(([dx, dz, s], i) => props.push(placeModel(scene, `q_rock_${1 + (i % 3)}`, ms.x + dx, ms.z + dz, { rot: i * 1.3, scale: s })));
  ...
  const game = createGame({ scene, camera, canvas, playerChar, npcs, alpaca, renderFrame, quality, sun, workbench, sigils, lights, waystones, archiveDoor });
  ...
  window.__dbg = { game, scene, camera, renderer, npcs, playerChar, THREE, SUN_DIR, quality, IMPOSTORS,
    ...
    cam(yaw, pitch, dist) { game.cam.free = null; game.cam.yaw = yaw; game.cam.pitch = pitch; game.cam.dist = dist; game.cam.dragT = 99; game.settle(); },
