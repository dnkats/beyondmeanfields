...
import { terrainH, resolveCollisions, surfaceH, cameraBlocked, WATER_Y, islandR } from './world.js';
...
export function createPlayerController(character, cam, keys, input, stick) {
  ...
  P.move = (dt, frozen, walkable) => {
    ...
    if (P.moving) {
      ...
      const can = (x, z) => { if (!walkable(x, z) || islandR(x, z) >= 1.03) return false; const h = surfaceH(x, z), th = terrainH(x, z), rock = h > th + 0.05;
